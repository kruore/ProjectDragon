    ...
    IEnumerator CalculateDistanceWithPlayer()
    {
        ...
        while (isActive)
        {
            ...
            if (DistanceCheckPlayerAndEnemy(player.GetComponent<Transform>(), other.GetComponent<Transform>()) < player.GetComponent<Player>().AtkRange)
            {
                ...
                player.enemy_angle = Quaternion.FromToRotation(Vector3.up, player.transform.position - other.transform.position).eulerAngles.z;
                player.current_angle = Quaternion.FromToRotation(Vector3.up, player.transform.position - player.joystickPos).eulerAngles.z;
            ...
            if (DistanceCheckPlayerAndEnemy(player.GetComponent<Transform>(), other.GetComponent<Transform>()) > player.GetComponent<Player>().AtkRange)
            {
                ...
                player.enemy_angle = Quaternion.FromToRotation(Vector3.up, player.transform.position - other.transform.position).eulerAngles.z;
              //  player.current_angle = Quaternion.FromToRotation(Vector3.up, player.joypadinput.transform.position - player.joystickPos).eulerAngles.z;
            }
